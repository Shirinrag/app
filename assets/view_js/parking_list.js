    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $.each(slot_info, function(slot_info_index, slot_info_row) {
                ...
                if(slot_info_row['bluetooth_device_status']==1){
                    bluetooth_device_status +='<label class="toggle"><input class="toggle-checkbox" type="checkbox" checked id="switch' + slot_info_row['bluetooth_device_status'] + '" onclick="bluetooth_device_status(' + slot_info_row['slot_info_id'] + ',' + slot_info_row['bluetooth_device_status'] + ',' + slot_info_row['fk_place_id'] + ');"><div class="toggle-switch"></div><span class="toggle-label"></span></label>';
                }else{
                    bluetooth_device_status +='<label class="toggle"><input class="toggle-checkbox" type="checkbox" id="switch' + slot_info_row['bluetooth_device_status'] + '" onclick="bluetooth_device_status(' + slot_info_row['slot_info_id'] + ',' + slot_info_row['bluetooth_device_status'] + ',' + slot_info_row['fk_place_id'] +  ');"><div class="toggle-switch"></div><span class="toggle-label"></span></label>';
                }
                html += "<div class='row'><input type='hidden' name='edit_slot_id[]' id='edit_slot_id_" + slot_info_index +"' value='"+slot_info_row['slot_info_id']+"' ><div class='col-md-2'><div class='form-group'><label>Slot Name</label><br><span class='data_fetch'>" + slot_info_row['slot_name'] + "</span></div></div><div class='col-md-2'><div class='form-group'><label>Display Id</label><br><span class='data_fetch'>" + slot_info_row['display_id'] + "</span></div></div><div class='col-md-2'><div class='form-group'><label>Machine Id</label><br><select name='fk_machine_id[]' id='fk_machine_id_"+slot_info_row['slot_info_id']+"' class='form-control chosen-select-deselect select_data'>'"+option_html+"'</select></div></div><div class='col-md-2'><div class='form-group'><label> Device Status</label><div>"+change_status+"</div></div></div><div class='col-md-2'><div class='form-group'><label> Delete Device</label><div>"+change_status1+"</div></div></div><div class='col-md-2'><div class='form-group'><label> Bluetooth Device Status</label><div>"+bluetooth_device_status+"</div></div></div><div class='col-md-4'><div class='form-group'><label> Bluetooth Device Name</label><input type='text' class='form-control input-text' placeholder='Bluetooth Device Name' name='bluetooth_device_name[]' id='bluetooth_device_name_"+slot_info_row['slot_info_id']+"' value='"+slot_info_row['bluetooth_device_name']+"'></div></div><div class='col-md-4'><div class='form-group'><label> Service Key</label><input type='text' class='form-control input-text' placeholder='Service Key'name='service_key[]' id='service_key_"+slot_info_row['slot_info_id']+"' value='"+slot_info_row['service_key']+"'></div></div></div>";
...
function bluetooth_device_status(id, status_1) {
    var status_1 = status_1;
    // console.log(status_1);return;
    if (status_1 == 1) {
        var bluetooth_device_status = 0;
    } else {
        var bluetooth_device_status = 1;
    }
    var user_id = id;
    //console.log(domain_id);
    $.ajax({
        url: frontend_path + "superadmin/update_bluetooth_device_status",
        type: "POST",
        data: {
            'id': user_id,
            'status': bluetooth_device_status
        },
        dataType: 'json',
