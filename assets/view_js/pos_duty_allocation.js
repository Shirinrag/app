...
$('#save_pos_vrifier_duty_allocation_form').submit(function(e) {
    e.preventDefault();
    var formData = new FormData($("#save_pos_vrifier_duty_allocation_form")[0]);
    ...
    jQuery.ajax({
        ...
        success: function(response) {
            ...
            if (response.status == 'success') {
                $('form#save_pos_vrifier_duty_allocation_form').trigger('reset');
                $(".chosen-select-deselect").val('');
                $('.chosen-select-deselect').trigger("chosen:updated");
                $('#pos_duty_data_append').html('');
                $('#pos_duty_allocation_table').DataTable().ajax.reload(null, false);
                swal({
                    title: "success",
                    text: response.msg,
                    icon: "success",
...
$(document).ready(function() {
    table = $('#pos_duty_allocation_table').DataTable({
        "ajax": frontend_path + "superadmin/display_all_pos_duty_allocation_data",
        "columns": [{
                "data": null
            },
            {
            ...
            },
            {
                ...
                "defaultContent": '<span><a href="javascript:void(0);" data-toggle="tooltip" class="mr-1 ml-1" title="Delete Details" class="remove-row"><i class="ti-trash a_delete_user" href="#delete_pos_duty_allocation" class="trigger-btn" data-bs-toggle="modal" data-bs-target="#delete_pos_duty_allocation" aria-hidden="true"></i></a></span>'
            },
        ],
        "order": [
            [0, 'desc']
...
$(document).on("click","#pos_duty_allocation_table tbody tr, .edit_duty_allocation tbody tr td",function(){
    var tr = $(this).closest('tr');
    var row = table.row(tr);
    var data1 = row.data();
  
    $('#delete_pos_duty_allocation_id').val(data1.id);
});
...
$("#delete-form").on('submit', (function(e) {
    e.preventDefault();
    $.ajax({
        url: frontend_path + "superadmin/delete_pos_duty_allocation",
        type: "POST",
        ...
        dataType: 'json',
        beforeSend: function() {
            $('#delete_pos_duty_allocation_button').button('loading');
        },
        success: function(data) {
            ...
            $("#delete_pos_duty_allocation").modal('hide');
            $('#delete_pos_duty_allocation_button').button('reset');
        }
    });
}));