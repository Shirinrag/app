...
$('#add_blogs_form').submit(function(e) {
    e.preventDefault();
    var formData = new FormData($("#add_blogs_form")[0]);
    ...
    jQuery.ajax({
        ...
        success: function(response) {
            ...
            if (response.status == 'success') {
                $('form#add_blogs_form').trigger('reset');
                // CKEDITOR.instances.editeur1.setData("");

                $('#blogs_data_table').DataTable().ajax.reload(null, false);
                swal({
                    title: "success",
                    text: response.msg,
                    icon: "success",
...
$(document).ready(function() {
    table = $('#blogs_data_table').DataTable({
...
$(document).on("click","#blogs_data_table tbody tr, .edit_blogs_details tbody tr td",function(){
